...
function NewBill(){
openwin('pandianbill.asp',900,600)
}
...
%>
<p align="left"><span class="STYLE1">盘点单查询</span></p>
<table align="left"><tr><td>
<form name="form1" method="post" action="selectpandian.asp">
<table border="0">
<tr>
...
<hr>
<input type="button" value="新增仓库盘点单" onMouseOver="this.className='button_over';" onMouseOut="this.className='button';"   class="button" onClick="NewBill();">
...
sDepotname = " and depotname in ("& s_depotpower &"'')"
...
End If
sql = "select distinct CONCAT('<a href=# onClick=checkbill(''',billcode+''','+convert(varchar(1),s_billdetail.check)+')>审核</a>&nbsp;&nbsp;<a href=# onClick=uncheckbill(''',billcode+''','+convert(varchar(1),s_billdetail.check)+')>反审</a>&nbsp;&nbsp;<a href=# onClick=EditBill(''',billcode+''','+convert(varchar(1),s_billdetail.check)+')>修改</a>&nbsp;&nbsp;<a href=# onClick=del(''',billcode+''','+convert(varchar(1),s_billdetail.check)+')>删除</a>&nbsp;&nbsp;<a href=detail.asp?billcode='+billcode+'>详情</a>' as action,billcode,adddate,custname,depotname,username,memo,checkman,case when s_billdetail.check=1 then '已审核' else '未审核' end as state from s_billdetail where AddDate<='"&s_date2&"' and AddDate>='"&s_date1&"' and billtype = '库存盘点'"&s_cust&sBillcode&sGoodscode&sGoodsname&sGoodsunit&sDepotname&" order by adddate desc,billcode desc"
