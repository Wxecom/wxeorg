...
<br>
<div align="left" class="style1">客户销售汇总表</div>
<table align="left"><tr><td>
<%
s_date1 = Year(Date()) & "-" & Month(Date()) & "-1"
s_date2 = formatdate(date)
%>
<form id="form1" name="sample" method="post" action="custsalecount.asp">
...
End If
'sql = "select s1.custname,ifnull(innum,0) as t_innum,ifnull(inmoney,0) as t_inmoney,ifnull(outnum,0) as t_outnum,ifnull(outmoney,0) as t_outmoney,ifnull(innum-outnum,0) as t_totalnum,ifnull(inmoney-outmoney,0) as t_totalmoney "
'sql = sql + "from (select custname from s_billdetail where 1=1"&s_custname&" group by custname) as s1 left join (select custname,sum(number) as innum,sum(money) as inmoney from s_billdetail where billtype='销售出库' and adddate>='"&s_date1&"' and adddate<='"&s_date2&"'"&s_depotname&s_goodsname&s_user&" group by custname) as s2 on s1.custname=s2.custname "
'sql = sql + "left join (select custname,sum(number) as outnum,sum(money) as outmoney from s_billdetail where billtype='销售退货' and adddate>='"&s_date1&"' and adddate<='"&s_date2&"'"&s_depotname&s_goodsname&s_user&" group by custname) as s3 on s1.custname=s3.custname "
sql = " select s1.custname,ifnull(innum,0) as t_innum,ifnull(inmoney,0) as t_inmoney,ifnull(outnum,0) as t_outnum,ifnull(outmoney,0) as t_outmoney,ifnull(innum-outnum,0) as t_totalnum,ifnull(inmoney-outmoney,0) as t_totalmoney from(select custname,sum(number) as innum,sum(money) as inmoney from s_billdetail where billtype='销售出库' and AddDate>='"&s_date1&"' and AddDate<='"&s_date2&"'"&s_depotname&s_username&s_goodsname&s_custname&"  group by custname) as s1 left join (select custname,sum(number) as outnum,sum(money) as outmoney from s_billdetail where billtype='销售退货' and AddDate>='"&s_date1&"' and AddDate<='"&s_date2&"'"&s_depotname&s_username&s_goodsname&s_custname&" group by custname) as s2 on s1.custname=s2.custname"

'response.write(sql)
call showpage(sql,"R_CustSaleCount",1)
endconnection
%>
</td></tr></table>
</div>
