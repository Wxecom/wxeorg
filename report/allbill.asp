...
sql = "select '<a href=# onClick=EditBill('''+billcode+''')>'+billcode+'</a>' as abillcode,billcode,custname,adddate,depotname,billtype,username,memo,case when t_bill.check=1 then '<font color=#0000FF>已审核</font>' else '<font color=#FF0000>未审核</font>' end as state,checkman from t_bill where (AddDate)<='"&s_date2&"' and (AddDate)>='"&s_date1&"'"&s_depotname&s_custname&s_goodsname&s_user&s_billcode&s_billtype&s_goodscode&s_check&" order by adddate desc"
'response.write(sql)
